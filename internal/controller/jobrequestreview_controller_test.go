...
var _ = Describe("JobRequestReview Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind JobRequestReview")
			err := k8sClient.Get(ctx, typeNamespacedName, jobrequestreview)
			if err != nil && errors.IsNotFound(err) {
				resource := &platformv1.JobRequestReview{
